...
- **Resolved (was a Phase 1 limitation):** the tree checkbox only ever reaches non-`unchanged` nodes, so a `new-file` build assembled purely through it could never include an identical (unchanged) UDT instance or folder — meaning a UDT definition with even one untouched member tag came out incomplete on export, breaking import into a gateway that doesn't have the type at all yet. This surfaced in real use exporting a UDT definition to a new target gateway. Fixed in Phase 2 with a distinct "force-include whole subtree" action (`allDescendants`/`toggleWholeSubtree` in `treeHelpers.ts`, surfaced as a button in the detail pane) — deliberately kept separate from the normal checkbox/`toggleSelection` rather than folded into it, so the everyday differing-only workflow is unchanged. Required zero engine changes: `buildMergePlan`'s `new-file` branch already handled an explicitly-selected `unchanged` path correctly once it's actually in the selection set.
